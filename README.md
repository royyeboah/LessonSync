...
A Spring Boot backend reads the image with Vertex AI (Gemini), stores the extracted lectures in
Redis so you can correct them, then creates a dedicated calendar and a recurring event per class in
your own Google account. The frontend is an Angular app.
...
refresh token that comes back is stored in Redis and used to mint access tokens automatically.
...
The session cookie only records which Google account you are; the tokens live in Redis under
`lessonsync:oauth:*`, keyed by Google account id. Because the refresh token is kept, closing the tab
and coming back later does not require consenting again, and the client library refreshes expired
